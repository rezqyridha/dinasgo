...
// ✅ Hanya admin dan bendahara yang boleh akses
...
$id = (int) ($_GET['id'] ?? 0);
if (!$id) {
    header("Location: " . BASE_URL . "/modules/shared/rincian_biaya/index.php?msg=invalid&obj=rincian");
    exit;
}

// ✅ Cek status disetujui/selesai
...
if (!$status || !in_array($status, ['disetujui', 'selesai'])) {
    header("Location: " . BASE_URL . "/modules/shared/rincian_biaya/index.php?msg=locked&obj=rincian");
    exit;
}
...
$pdf->SetX(20); // mulai dari margin kiri 20 mm (standar)
...
$pdf->Cell(0, 7, '__________________________', 0, 1, 'R');
