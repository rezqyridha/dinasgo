...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($input['nama'] === '' || $input['nip'] === '' || $input['jabatan'] === '') {
        ...
        // Cek NIP duplikat
        $cek = $conn->prepare("SELECT id FROM kepala WHERE nip = ?");
        $cek->bind_param("s", $input['nip']);
        $cek->execute();
        $cek->store_result();
        if ($cek->num_rows > 0) {
            ...
            $stmt = $conn->prepare("
                INSERT INTO kepala (nama, nip, jabatan, tahun)
                VALUES (?, ?, ?, ?)
            ");
            $stmt->bind_param(
                "ssss",
            ...
            if ($stmt->execute()) {
                header("Location: index.php?msg=added&obj=kepala");
                exit;
            } else {
                $error = "Gagal menyimpan data kepala.";
            }
            $stmt->close();
        }
        $cek->close();
